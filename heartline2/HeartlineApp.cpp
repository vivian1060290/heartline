...
void ChartWidget::paintEvent(QPaintEvent *event) {
    ...
    painter.rotate(-90);
    painter.drawText(0, 0, "Emotion Score (0-10)");
    painter.restore();

    // Draw X-axis label
    painter.drawText(width() / 2, height() - 10, "Date");
    ...
    for (int i = 0; i <= 10; ++i) {
        ...
        painter.drawText(30, y, QString::number(i));
...
HeartlineApp::HeartlineApp(QWidget *parent)
    : QMainWindow(parent) {
    // Initialize UI components and setup Login UI
    setupLoginUI();
}
...
void HeartlineApp::setupSettingsUI()
{
    ...
    QPushButton *viewBarChartButton = new QPushButton("View Bar Chart", &settingsDialog);
    ...
    settingsLayout->addWidget(viewBarChartButton);
    ...
    connect(viewBarChartButton, &QPushButton::clicked, this, &HeartlineApp::setupBarChartUI);
    // 顯示設定視窗
    settingsDialog.exec();
}
...
void HeartlineApp::setupBarChartUI() {
    QDialog barChartDialog(this);
    barChartDialog.setWindowTitle("Bar Chart");

    barChartDialog.resize(800, 600);
    // Load data
    QMap<QDate, int> dateEmotionCounts;
    QDir userDir("diary/" + currentUsername);
    QStringList files = userDir.entryList(QStringList("*.csv"), QDir::Files);
    for (const QString &fileName : files) {
        QFile file(userDir.filePath(fileName));
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            while (!in.atEnd()) {
                ...
                if (parts.size() >= 3) {
                    QDate date = QDate::fromString(parts[0], "yyyy-MM-dd");
                    if (date.isValid()) { // Ensure date is valid
                        int emotion = parts[1].toInt();
                        dateEmotionCounts[date] = emotion;
                    }
                }
            }
        }
    ...
    QBarSet *barSet = new QBarSet("Emotion Score");
    for (auto it = dateEmotionCounts.begin(); it != dateEmotionCounts.end(); ++it) {
        categories << it.key().toString("MM-dd"); // Add date to categories
        *barSet << it.value();                   // Add emotion score to the bar set
    }

    // Create bar chart
    QBarSeries *series = new QBarSeries();
    series->append(barSet); // Add bar set to series

    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("Emotion Distribution by Date");
    ...
    axisY->setRange(0, 10);
    axisY->setTitleText("Emotion Score");
    axisY->setTickCount(11); // Ensure integer ticks from 0 to 10
